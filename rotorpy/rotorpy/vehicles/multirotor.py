...
class Multirotor(object):
    ...
    def _pack_state(cls, state):
        ...
        # reshape state["x"] from (3,1) to (3,) to avoid errors
        if isinstance(state['x'], np.ndarray):
            state_x =  state['x'].reshape(-1)  # Standard NumPy reshape
        elif isinstance(state['x'], ca.DM):  # CasADi's DM object
            state_x =  state['x'].full().reshape(-1)  # Convert to NumPy first
        else:
            raise TypeError(f"Unsupported type for reshaping: {type(state)}")

        if isinstance(state['v'], np.ndarray):
            state_v =  state['v'].reshape(-1)  # Standard NumPy reshape
        elif isinstance(state['v'], ca.DM):  # CasADi's DM object
            state_v =  state['v'].full().reshape(-1)  # Convert to NumPy first
        else:
            raise TypeError(f"Unsupported type for reshaping: {type(state)}")


        s[0:3]   = state_x # inertial position
        s[3:6]   = state_v       # inertial velocity
