...
class SE3Control(object):
    ...
    def update(self, t, state, flat_output):
        ...
        pos_err  = state['x'] - flat_output["x"] # x = flat_output[0]
        dpos_err = state['v'] - flat_output["x_dot"] # x_dot = flat_output[1]
        F_des = self.mass * (- self.kp_pos*pos_err
                             - self.kd_pos*dpos_err
                             + flat_output["x_ddot"] # x_ddot = flat_output[2]
                             + np.array([0, 0, self.g]))
        ...
        yaw_des = flat_output["yaw"]   #yaw = flat_output[3]
        ...
        w_des = np.array([0, 0, flat_output["yaw_dot"]]) #yaw_dot = flat_output[4]
        w_err = state['w'] - w_des
