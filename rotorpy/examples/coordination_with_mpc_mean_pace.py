...
def execute_mpc(trajectories):
    ...
    gamma_dot_all_new = gamma_dot_all.copy()
    ...
    while True:
        ...
        gamma_dot_all = gamma_dot_all_new.copy()
        min_dist = np.inf
        for i in range(num_agents):
            ...
            if with_delay:
                ...
                u[t, :, i], cost[t, i] = mpc.solve(x[t, :, i], gamma_all, gamma_dot_all, x_max, x_min, u_max, u_min, actual_state, i, L)


            x[t + 1, :, i] = A @ x[t, :, i] + B @ u[t, :, i]
            approx_x = A @ mpc.x_buffer[-1][:, -1]
            gamma_all_new[i, :] = np.hstack([mpc.x_buffer[-1][0, 1:], approx_x[0]])
            gamma_dot_all_new[i, :] = np.hstack([mpc.x_buffer[-1][1, 1:], approx_x[1]])
            end_time = time.time()
